...
if(strncmp(msg, comp, 5)==0){
  
  //printf("\nChanging username... \n");

int i = 6;
int index=0;
memset(client->username,0,UserLength);
//printf("%d", strlen(client->username));
while(i < strlen(msg)){
  //printf("%c", msg[i]);
  client->username[index] = msg[i];
  i++;
  index++;
...
client->username[index] = '\0';
//i = 0;
//while(client->username[i]!='\0'){
//	printf("%c", client->username[i]);
//	i++;
//}
//printf("%d", strlen(client->username));
UserLength = strlen(client->username);
continue;
}
